    ...
    <div class="search-form">
      <a-form :model="form" layout="inline">
        ...
        <a-form-item label="状态">
          <a-input v-model:value="form.status" placeholder="请输入状态" allow-clear />
        </a-form-item>
        <a-form-item label="IP">
          <a-input v-model:value="form.ip" placeholder="请输入IP" allow-clear />
        </a-form-item>
        <a-form-item>
          <a-space>
...
const handleSearch = async (params: TablePaginationConfig = { current: 1, pageSize: 10 }) => {
  ...
  try {
    ...
    if (form.status) queryParams.append('status', form.status);
    if (form.ip) queryParams.append('ip', form.ip);

    const res = await http.get(`/service/?${queryParams.toString()}`);
